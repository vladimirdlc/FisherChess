    ...
    public boolean checkRouteTo(int x, int y, Square[][] squares)
    {
        ...
        if(mov.contains(posObjetive))
        {
            if(x > this.getX() && y > this.getY())
            {
                for(int i = x-this.getX()-1; i > 0; i--)
                {
                    if(!mov.contains(new Vector2(this.getX()+i,this.getY()+i))
                            || !squares[this.getY()+i][this.getX()+i].isEmpty()) validated = false;
                }
            }
            else if(x < this.getX() && y < this.getY())
            {
                for(int i = this.getX()-x-1; i > 0; i--)
                {
                    if(!mov.contains(new Vector2(x+i,y+i))
                            || !squares[y+i][x+i].isEmpty()) validated = false;
                }
            }
            else if(x < this.getX() && y > this.getY())
            {
            ...
            else if(x > this.getX() && y == this.getY())
            {
                ...
                {
                    if(!mov.contains(new Vector2(i,y)) || !squares[y][i].isEmpty()) validated = false;
                }
            }
            else if(x < this.getX() && y == this.getY())
            {
